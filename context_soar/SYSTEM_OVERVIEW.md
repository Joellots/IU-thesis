...
Workflow: Kafka `alert.translated` trigger → fetch full alert from Postgres → parse alert
→ severity (by `pred_proba`) → **High/Medium:** normalize/dedupe observables, skip non-global
IPs by default, run bounded pre-case Cortex+MISP enrichment → combine ML+intel+XAI → decision
matrix → TheHive case / Shuffle action (auto-block only on High+confirmed-IOC; **isolate
always human-gated**) → async analyst feedback to PostgreSQL (retraining labels). **Low
(<0.70): notify only.** Slow Cortex analyzers may appear as `pending` in the initial case if
`CORTEX_PRE_CASE_WAIT_SEC`/`CORTEX_WAIT_SECONDS` expires before the report is ready.
...
## 5. Integration status / sync items (2026-06)

1. **JA3/JA3S observable wiring — DONE.** NFStream 6.6.0 exposes
   `client_fingerprint` (JA3) and `server_fingerprint` (JA3S). The translator's
   `extract_observables()` now emits `{type: "ja3", value, role}` for fingerprint fields so
   Cortex/MISP can correlate them (MISP `ja3-fingerprint-md5`).
...
- Dashboard `:8080`, Kafka-UI `:8081`, Kafka external `:9094` (`KAFKA_EXTERNAL_ADVERTISED_HOST` must be the detection host IP/DNS for remote SOAR consumers; do not advertise `localhost` to the SOAR host).
