...
**Last updated:** 2026-06-19. **For the whole-project picture (both halves) and the cross-half
...
The classifier has been **retrained on NFStream features**, the dataset rebuilt, the
**feature→MITRE mapping validated and made live**, and Kafka-triggered SOAR handoff is wired.
Remaining detection-side work is small (Phase-4 literature grounding) plus the end-to-end
demo and thesis writing.
...
- **JA3 observable extraction [DONE]** — `services/translator/translator_service.py`
  extracts fingerprint fields as `{type: "ja3", value, role}` so the SOAR side can route TLS
  fingerprints to Cortex/MISP.
...
- `services/translator/translator_service.py` — consumes `alerts`, calls `translate()`, extracts `observables` (`ip`/`domain`/`url`/`ja3`), writes PostgreSQL `alerts` (incl. `mapping_*`), then publishes a thin `alert.translated` Kafka event to `SOAR_ALERT_EVENTS_TOPIC` (default `soar_alert_events`) after the insert commits.
...
| postgres | `postgres` | shared `alerts` store (the SOAR contract; 5432 exposed/firewalled for cross-machine SOAR) |
...
> Kafka `9094` with `KAFKA_EXTERNAL_ADVERTISED_HOST=<DETECTION_HOST_IP>` so broker metadata is reachable from the SOAR host (not `localhost`), and point the orchestrator's `DATABASE_URL` / `KAFKA_BROKER` at this machine's IP.
