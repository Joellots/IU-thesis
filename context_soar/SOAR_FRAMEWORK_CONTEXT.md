...
**Deployment topology (current, real IPs in `.env`):**
...
- `_parse_observables()` **normalizes, deduplicates, and filters**:
  ...
  - **private-IP skip:** `_should_skip_observable()` drops non-global IPs **iff**
    `SKIP_PRIVATE_IP_OBSERVABLES=true`. The **code default is `true`**, but the **current
    `.env` sets it to `false`**, so private IPs are presently kept (relevant when reading live
    cases that show RFC1918 targets). JA3/domain/url are never skipped.
...
| ≥ 0.90 | **High** |
| 0.70–0.89 | **Medium** |
| < 0.70 | **Low** (analyst-review-only / notify-only) |

The orchestrator is the **single source of truth** for severity; `severity_label` from the
translator is advisory (kept in the case for traceability as `soarSeverityAdvisory`).
...
- **URLhaus**: `query_status=ok` with a `threat`/`payloads`/`url_status` ⇒ `malicious` even
  when the summary taxonomy is empty; `no_results` ⇒ `info`.
...
- **Block/Isolate → Wazuh on-demand Active-Response** (`wazuh_response.py`): the orchestrator
  ...
  `{"command":"!soar-block","arguments":["<dst_ip>"]}`; the agent runs the vetted local
...
- **Start the SOAR stack:** `scripts/soarctl.sh start core` (TheHive + **Cortex** + MISP +
  Shuffle). soarctl manages the vendored stacks under `deploy/soar/{thehive,misp,shuffle,
  wazuh}` as separate compose projects.
...
- **The `!soar-block` command prefix is unconfirmed until the first real dispatch** (agent
  `000` isn't a valid AR target); configurable via `WAZUH_AR_COMMAND_PREFIX`.
- **Dashboard approval UI is a detection-side build** — the SOAR contract
  (`soar_pending_approvals` + `/soar/approve`) is ready; the list/buttons are pending there.
  Until then, gated actions sit `pending` and expire after the TTL.
- **Step 6 is interim** on the orchestrator (`/soar/feedback` → `soar_analyst_feedback`);
  destined for the dashboard + shared columns.
- **Auto-block rarely fires on synthetic data:** confirmation analysts (MISP/VT/URLhaus) must
  flag an observable malicious for the auto-block cell — seed an IOC in MISP to exercise it.
  Most synthetic High alerts land in the **gated** cell (now → dashboard approval). Expected.
...
- **Detection-owned files are off-limits** to the SOAR side (`services/{nfstream,inference,
  translator,producer,dashboard}`, `utils/`, `model_training/`, `schema.sql`); cross-half
  changes are coordination items.
...
| `api_server.py` | Flask `/soar/shuffle-result` + `/soar/feedback` |
...
- **Severity** — High ≥0.90 / Medium 0.70–0.89 / Low <0.70 (SOAR-authoritative).
