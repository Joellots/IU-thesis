...
> ⚠️ **UPDATE — the approval gate is RETIRED and enforcement is real.** The `parse` node now
> always routes to `execute_direct`, so the **User-Input gate below is unreachable** (a gated
> payload runs `parse → execute_direct → FINISHED`). Shuffle is now a **recorder/visualizer**
> only. The real enforcement + approval moved to the orchestrator:
> - **notify** → Slack (`notify_client.py`); **block/isolate** → Wazuh on-demand AR
>   (`wazuh_response.py`), routed to the endpoint's agent.
> - **gated** block/isolate → the **dashboard approval loop** (`soar_pending_approvals` +
>   token-gated `POST /soar/approve` → real Wazuh AR), see `SOAR_WORKFLOW_SPEC.md` §6.
>
> The graph/notes below are retained as historical reference for the original gated design.
